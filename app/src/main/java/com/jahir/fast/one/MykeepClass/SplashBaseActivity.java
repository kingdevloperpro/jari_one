...
import static com.jahir.fast.one.in_ex.one_Ex_Clinet.getGeoApiService;
...
import com.jahir.fast.one.in_ex.one_Ex_Res;
import com.jahir.fast.one.in_ex.one_Ex_Serc;
...
public abstract class SplashBaseActivity extends AppCompatActivity {
    ...
    private void verifyUser(List<LocArray> locArray) {
        one_Ex_Serc apiService = getGeoApiService();
        apiService.getLocation().enqueue(new retrofit2.Callback<one_Ex_Res>() {
            @Override
            public void onResponse(Call<one_Ex_Res> call, Response<one_Ex_Res> response) {
            ...
            @Override
            public void onFailure(Call<one_Ex_Res> call, Throwable t) {
